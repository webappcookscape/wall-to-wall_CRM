...
const Dashboard: React.FC = () => {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="max-w-md rounded-2xl border border-red-100 bg-red-50 p-6 text-center shadow-sm">
          ...
          <button
            onClick={fetchData}
            className="mt-4 rounded-xl bg-brand px-6 py-2.5 text-xs font-bold uppercase tracking-wider text-white hover:bg-[#004d30] transition-colors"
          >
            Retry
          </button>
        </div>
  ...
  const statTiles = [
    { label: 'Fresh Leads', value: stats?.freshlead || 0, icon: <LayoutGrid />, path: '/leadhub' },
    { label: 'Yet To Follow-up', value: stats?.yettofollow || 0, icon: <Bell />, path: '/leads' },
    { label: 'In Follow-ups', value: stats?.followup || 0, icon: <Users />, path: '/leads' },
    { label: 'Opportunities', value: stats?.opportunities || 0, icon: <TrendingUp />, path: '/leads' },
    { label: 'Order Booked', value: stats?.orderbook || 0, icon: <ShoppingCart />, path: '/leads' },
    { label: 'Disqualified', value: stats?.disqualified || 0, icon: <XCircle />, path: '/leads' },
  ];
  ...
  return (
    <div className="container-fluid py-4 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5 flex-wrap">
            <h4 className="text-2xl sm:text-3xl font-extrabold text-gray-800 tracking-tight">
            ...
            {selectedUserId && activeEmployeeObj ? (
              <span className={`text-xs uppercase font-bold px-2.5 py-1 rounded-full border ${getRoleBadgeClass(activeEmployeeObj.role || '')}`}>
                {formatRoleLabel(activeEmployeeObj.role || '')}
              </span>
            ) : (
              <span className={`text-xs uppercase font-bold px-2.5 py-1 rounded-full border ${getRoleBadgeClass(user?.role || '')}`}>
        ...
        <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
          ...
          {isManagerOrAdmin && (
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2 focus-within:ring-2 focus-within:ring-brand/10 focus-within:border-brand transition-all">
      ...
      {selectedUserId && activeEmployeeObj && (
        <div className="bg-brand/5 border border-brand/20 p-4 rounded-2xl flex items-center justify-between gap-3 animate-in fade-in duration-200">
          ...
          <button
            ...
            className="text-xs font-bold text-brand hover:underline px-3.5 py-1.5 rounded-lg hover:bg-brand/10 transition-colors"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {statTiles.map((tile, idx) => (
          <Link 
            key={idx} 
            to={tile.path}
            className="group relative bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 hover:shadow-lg hover:border-gray-200 transition-all duration-200 overflow-hidden flex flex-col justify-between"
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="w-9 h-9 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 group-hover:text-brand group-hover:bg-brand/10 transition-all [&_svg]:w-5 [&_svg]:h-5">
                {tile.icon}
              </div>
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Reminders Card */}
        <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6 shadow-sm flex flex-col">
          <div className="flex items-center justify-between pb-4 border-b border-gray-100 mb-4">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-red-50 text-red-600 flex items-center justify-center">
                <Bell size={17} />
              </div>
              <h5 className="font-bold text-base text-gray-800 m-0">Pending Reminders</h5>
            </div>
            <span className="bg-red-50 text-red-600 text-xs font-black px-2.5 py-0.5 rounded-full border border-red-100">
            ...
            {stats?.upcomingReminders && stats.upcomingReminders.length > 0 ? (
              <div className="space-y-3">
                {stats.upcomingReminders.map(rem => (
                  <div 
                    key={rem.id} 
                    className="p-3 rounded-xl bg-gray-50 hover:bg-gray-100/80 transition-colors border border-gray-100 flex items-center justify-between gap-2.5"
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-gray-800 truncate">{rem.name}</span>
                        <span className="text-xs font-bold text-gray-400">#CUST-{rem.leadId}</span>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
                        <span className="flex items-center gap-1 text-brand font-medium">
                          <Phone size={12} /> {rem.phone}
                        </span>
                        {rem.project?.name && (
                    ...
                    {rem.contactableDate && (
                      <span className="text-xs font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-100 shrink-0">
              ...
              <div className="h-48 flex flex-col items-center justify-center text-center text-gray-400 gap-2">
                <CheckCircle2 size={36} className="text-emerald-400 opacity-60" />
                <p className="text-sm font-medium text-gray-500">All reminders are up to date</p>
              </div>
            )}

            <Link 
              to="/reminders" 
              className="mt-4 pt-3.5 border-t border-gray-50 text-xs font-bold text-brand hover:text-[#004d30] flex items-center justify-center gap-1.5 transition-colors uppercase tracking-wider"
            >
              Open Reminders Hub <ArrowRight size={14} />
            </Link>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between pb-4 border-b border-gray-100">
            <div className="flex items-center gap-2.5">
              ...
              <h5 className="font-bold text-base text-gray-800 m-0">Team Lead Allocations</h5>
            </div>
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Active Queue</span>
          </div>
            ...
            {[
              ...
              <div key={i} className="p-3.5 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">{item.role}</span>
                </div>
                <span className="text-xl font-black text-gray-800">{item.value.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 sm:p-6 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between pb-4 border-b border-gray-100">
            <div className="flex items-center gap-2.5">
              ...
              <h5 className="font-bold text-base text-gray-800 m-0">Pipeline Stage Distribution</h5>
            </div>
          ...
          <div className="pt-2.5 flex justify-between items-center text-xs text-gray-500 font-bold border-t border-gray-50">
          ...
          <div className="p-5 sm:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              <div className="flex items-center gap-2.5">
                ...
                <h5 className="font-bold text-lg text-gray-800 m-0">Employee Performance Breakdown</h5>
              </div>
            ...
            <div className="w-full sm:w-72">
              <input
                ...
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm font-medium outline-none focus:ring-2 focus:ring-brand/10 focus:border-brand transition-all"
              />
            </div>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-50/75 border-b border-gray-100 text-xs font-bold text-gray-500 uppercase tracking-wider">
                <tr>
                  ...
                  <th className="py-3.5 px-3 text-center">Yet To Follow</th>
                  <th className="py-3.5 px-3 text-center">In Follow-up</th>
                  <th className="py-3.5 px-3 text-center">Opportunities</th>
                  <th className="py-3.5 px-3 text-center">Won (Booked)</th>
                  ...
                  return (
                    ...
                    >
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-brand to-emerald-400 text-white font-bold text-sm flex items-center justify-center shrink-0">
                            {emp.fullName.charAt(0)}
                          </div>
                          <div className="min-w-0">
                            <p className="font-bold text-gray-800 text-sm truncate">{emp.fullName}</p>
                            <p className="text-xs text-gray-400 truncate">{emp.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-3.5 px-3">
                        <span className={`text-xs uppercase font-bold px-2.5 py-0.5 rounded-full border ${getRoleBadgeClass(emp.role)}`}>
                          {emp.role}
                        </span>
                      </td>
                      <td className="py-3.5 px-3 text-center font-bold text-gray-700 text-sm">
                        {emp.totalAssigned}
                      </td>
                      <td className="py-3.5 px-3 text-center">
                        <span className={`font-bold text-sm ${emp.yettofollow > 0 ? 'text-amber-600' : 'text-gray-400'}`}>
                          {emp.yettofollow}
                        </span>
                      </td>
                      <td className="py-3.5 px-3 text-center font-bold text-indigo-600 text-sm">
                        {emp.followup}
                      </td>
                      <td className="py-3.5 px-3 text-center font-bold text-cyan-600 text-sm">
                        {emp.opportunities}
                      </td>
                      <td className="py-3.5 px-3 text-center">
                        <span className="bg-emerald-50 text-emerald-700 font-bold px-2.5 py-1 rounded-md border border-emerald-100 text-xs">
                          {emp.orderbook}
                        </span>
                      </td>
                      <td className="py-3.5 px-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          <span className="font-extrabold text-gray-800 text-sm">{emp.conversionRate}%</span>
                        </div>
                      </td>
                      <td className="py-3.5 px-4 text-right">
                        <button
                          onClick={() => handleSelectEmployee(isSelected ? '' : emp.id)}
                          className={`text-xs font-bold uppercase tracking-wider px-3.5 py-1.5 rounded-lg transition-all ${
                            isSelected 
